...
from app.models.index import DbCustomer, DbWalletTransaction, DbPin, DbGame
...
def create_new_game(request: Game, db: Session = Depends(get_db)):
    ...
    first_price = request.game_winner_price * 50 / 100 # type: ignore
    second_price = request.game_winner_price * 30 / 100 # type: ignore
    third_price = request.game_winner_price * 20 / 100 # type: ignore
    new_game = DbGame(
        game_unique_id='MC'+uuid.uuid1().hex[:8],
        game_name=request.game_name,
        game_image=request.game_image,  
        game_end_date_time=request.game_end_date_time,
        game_unit_price= request.game_unit_price,
        game_min_ticket_number=request.game_min_ticket_number,
        game_1st_price=first_price,
        game_2nd_price=second_price,
        game_3nd_price=third_price,
        game_registration_date_time=datetime.now(pytz.timezone('Asia/Calcutta'))
    )
    db.add(new_game)
    db.commit()
    return {'status': 'Success', 'details': 'New Game Created', 'Creation Time': datetime.now(pytz.timezone('Asia/Calcutta'))}
...
def show_all_game(db: Session = Depends(get_db)):
    game = db.query(DbGame).all()
    if not game:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Game is not available")
    else:
        return game
...
#     cust = db.query(DbCustomer).filter(DbCustomer.cust_mobile == request.cust_mobile).first()
#     otp_valid_datetime = datetime.now(pytz.timezone('Asia/Calcutta'))+ timedelta(minutes=10)
